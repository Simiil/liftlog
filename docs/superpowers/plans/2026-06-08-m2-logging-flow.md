...
Executed via subagent-driven-development (fresh implementer per task; model-tiered haiku/sonnet/opus; two-stage/holistic opus reviews on the substantive groups). 19 tasks → 22 commits (incl. plan + 3 review-fix commits). Final state: 121 JVM unit tests + the instrumented suite (DAO + seeder + critical UI test) compiling; `lint testDebugUnitTest assembleDebug assembleDebugAndroidTest` all green locally; instrumented tests run in CI (no local KVM).

**Execution deviations from the plan as written:**
1. **Strings added per-screen, not front-loaded in T9.** Front-loading every M2 string would trip `lint` UnusedResources until the consuming screen landed; instead T9 added only the routes, and each screen task added the strings it references — keeping lint green continuously.
2. **`sessionId` read via `savedStateHandle.get<String>("sessionId")`** (the key type-safe route fields land under) rather than `toRoute<…>()`, for trivial pure-JVM testability. Used in `ActiveSessionViewModel` and `SessionDetailViewModel`.
3. **T13+T14 built in one dispatch (two commits)** and **T15+T16 in one dispatch (two commits)** — tightly-coupled screens, committed granularly (the M1 efficiency pattern).

**Review-driven fixes (the gates earned their keep):**
4. **Data-layer review (T4–T6) → `166c193`:** the set-count query's `se.deletedAt IS NULL` clause was entirely untested (a regression would have passed CI) — added a discriminating case; added the missing `s.deletedAt IS NULL` JOIN to the set-count query for parity/defense-in-depth (+ test); made the recency-ordering test sensitive to a dropped `ORDER BY`; fixed a self-contradictory test comment.
5. **Active Session review (T12–T14) → `8f343df`:** (a11y, **Important**) the logged-set edit was only reachable by long-press while the content description said "double-tap to edit" → added a `CustomAccessibilityAction("Edit set")` + fixed wording; (**Important**) `pendingReplaceSeId` wasn't persisted → process death during the picker silently turned *Replace* into *Add* (violates the no-data-loss constraint) → persisted in `SavedStateHandle`; finish snackbar was dead code (scope cancelled on pop) → made briefly visible; finish summary hardcoded English → string resources.
6. **Final holistic review → `232f0ac`:** (**Critical**) `SessionDetailScreen` keyed its `LazyColumn` by `name+equipment`, which collides when the same exercise appears twice in a session (a spec-supported case) → crash on opening that detail → keyed by `sessionExerciseId`; (**Important**) `HistoryScreen` hand-rolled relative dates with hardcoded English + wrong singulars → `DateUtils.getRelativeTimeSpanString` (as Home does); removed unused `session_finish`/`session_discard` strings; `set_count` → `<plurals>`; extracted `LoggedSetRow` literals to resources + added RPE/note to its content description; Session Detail now shows the date + loading/empty states; guarded a `startOrResume` race that could throw uncaught.

**Decisions flagged at the top, as-built (none rejected at the gate):** units KG-only UI + conversion layer ready (#1); `lastPerformance` suspend snapshot (#2); `replaceExercise` = remove-old + add-new-at-same-position (#3); exercise name/equipment joined in ViewModels via `observeAll()` (#4); History shows set count not PR count (#5); recency via `MAX(completedAt)` query (#6); picker result via `savedStateHandle` (#7); M3-baseline visuals, tokens slot in later (#8); target-driven auto-advance implemented but dormant (#9); empty sessions = "Quick workout" (#10); process-death tested via `recreate()` + in-memory `@Singleton` Room behind a Hilt harness (#11).

### Forward-looking notes for later milestones
- **M3 (Plans):** the dormant pieces are wired and ready to light up — `ActiveSessionViewModel`'s target-driven auto-advance, the `targetSets`/`targetRepsMin`/`targetRepsMax` snapshot fields, and `RepsStepper(targetHint=…)`. Add template→session snapshot start, Home template chips, and the rep-range hint. **Caveat:** `replaceExercise` currently nulls the new slot's targets (fine for ad-hoc); a *prescribed-slot* replace in M3 should carry/re-snapshot targets. (Also carried from M1: `PlanRepositoryImpl.createPlan` should set `position = max+1`.)
- **M4 (Analytics):** add PR count to the History cards and the finish snackbar; build `AnalyticsRepository` over the existing `AnalyticsDao`; charts live-update via the existing reactive flows.
- **M5 (Export/import + polish):** ship the kg/lb **toggle** — the read-side `SettingsRepository.weightUnit` + the whole `Weights` conversion layer are already in place and used by the UI, so M5 only adds a setter + one Settings row; `BackupRepository`; consider moving the finish summary to a Home-hosted snackbar (currently a brief on-session snackbar); the critical UI test uses the real file-backed DataStore, so when a unit setter exists, reset it between instrumented tests.
- **General:** the two-quantity `home_resume_summary` ("%d exercises · %d min") isn't expressible as a single Android `<plurals>` — left as-is; revisit if strict i18n requires per-quantity pluralization.
