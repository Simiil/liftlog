...
class SessionLoggingDaoTest {
    ...
    @Test fun maxSetPosition_returnsNullWhenNoSetsExist() = runTest {
    ...
    @Test fun maxExercisePosition_returnsNullWhenNoExercisesExist() = runTest {
        dao.insertSession(session("s1"))
    ...
    @Test fun maxExercisePosition_returnsMaxNotInsertionOrder() = runTest {
    ...
    @Test fun maxSetPosition_ignoresSoftDeletedSets() = runTest {
        exerciseDao.insert(exercise("ex1"))
        dao.insertSession(session("s1"))
        dao.insertSessionExercise(sessionExercise("se1", "s1", "ex1"))
    ...
    @Test fun softDeleteLoggedSet_tombstonesBothTimestampsAndLeavesSiblingUntouched() = runTest {
        exerciseDao.insert(exercise("ex1"))
        dao.insertSession(session("s1"))
        dao.insertSessionExercise(sessionExercise("se1", "s1", "ex1"))
        dao.insertLoggedSet(loggedSet("ls1", "se1", position = 1))
    ...
    @Test fun softDeleteLoggedSetsForSessionExercise_cascadesOnlyToTargetExercise() = runTest {
        exerciseDao.insert(exercise("ex1"))
        dao.insertSession(session("s1"))
