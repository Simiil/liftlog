...
class SessionDaoTest {
    ...
    private fun exercise(id: String) =
        ExerciseEntity(id, "Exercise $id", MuscleGroup.BACK, Equipment.BARBELL,
    ...
    @Test fun observeActiveSession_emitsLiveSession_thenNullWhenEnded() = runTest {
    ...
    @Test fun observeHistory_excludesTombstones_orderedByStartedAtDesc() = runTest {
    ...
    @Test fun observeHistory_includesInProgressSessions() = runTest {
    ...
    @Test fun observeSessionWithDetails_returnsGraphShapeWithExercisesAndSets() = runTest {
    ...
    @Test fun observeSessionWithDetails_returnsNullForTombstonedSession() = runTest {
        dao.insertSession(session("s1", deleted = 99L))
        dao.observeSessionWithDetails("s1").test {
    ...
    @Test fun softDeleteLoggedSetsForSession_tombstonesAllSetsForSession() = runTest {
        exerciseDao.insert(exercise("ex1"))
        dao.insertSession(session("s1"))
    ...
    @Test fun softDeleteSessionExercisesFor_tombstonesOnlySessionsExercises() = runTest {
        exerciseDao.insert(exercise("ex1"))
        dao.insertSession(session("s1"))
        dao.insertSession(session("s2"))
        dao.insertSessionExercise(sessionExercise("se1", "s1", "ex1", position = 1))
