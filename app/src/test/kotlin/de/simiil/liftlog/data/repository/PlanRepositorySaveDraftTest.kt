...
class PlanRepositorySaveDraftTest {
    ...
    private fun liveExercises(templateId: String) =
        planDao.templateExercises.values
    ...
    @Test
    fun `savePlanDraft creates a new plan with days, exercises, positions and targets`() =
        runTest {
            val draft =
                PlanDraft(
                    planId = null,
    ...
    @Test
    fun `savePlanDraft on existing plan reorders, edits, removes - preserving ids and reindexing`() =
        runTest {
            // Seed: plan with day A (ex te-a1, te-a2) and day B (ex te-b1).
            planDao.plans["plan-1"] =
                WorkoutPlanEntity(
                    id = "plan-1",
                    name = "Old name",
                    position = 0,
            ...
            planDao.templateExercises["te-a1"] =
            ...
            planDao.templateExercises["te-a2"] =
                TemplateExerciseEntity(
                    id = "te-a2",
                    templateId = "day-A",
                    exerciseId = "ex-2",
            ...
            planDao.templateExercises["te-b1"] =
                TemplateExerciseEntity(
                    id = "te-b1",
                    templateId = "day-B",
                    exerciseId = "ex-9",
    ...
    @Test
    fun `savePlanDraft soft-deletes a day removed from the draft and cascades to its exercises`() =
        runTest {
            planDao.plans["plan-1"] =
                WorkoutPlanEntity(
                    id = "plan-1",
    ...
    @Test
    fun `not calling savePlanDraft leaves the database untouched`() =
        runTest {
            planDao.plans["plan-1"] =
                WorkoutPlanEntity(
                    id = "plan-1",
    ...
    @Test
    fun `savePlanDraft removing a day does not touch snapshotted session_exercises`() =
        runTest {
            ...
            sessionDao.sessionExercises["se-1"] =
